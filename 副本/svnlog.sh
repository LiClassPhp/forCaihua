...
while [ "$(date -d "$current_date" +%s)" -le "$end_date_ts" ]; do
    ...
    for repo in "${SVN_REPOS[@]}"; do
        ...
        # 获取原始日志输出
        log_output=$(svn log --username "$username" --password "$passwd" -r "{$current_date}:{$next_date}" -v --search "SinsnaZhou" "$repo" 2>&1)
